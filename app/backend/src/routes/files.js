...
filesRouter.post("/draft", requireRole("operator"), (req, res) => {
  ...
  try {
    ...
    recordAudit(req, "file.draft", id, "success");
    res.json({
      ok: true,
      id,
    ...
    recordAudit(req, "file.draft", id, "failure", { error: err.message });
...
filesRouter.post("/write", requireRole("admin"), (req, res) => {
  ...
  try {
    ...
    const stats = fs.statSync(file.path);
    recordAudit(req, "file.write", file.path, "success", { id, source: source || "editor" });
    res.json({
      ok: true,
      id,
    ...
    recordAudit(req, "file.write", id, "failure", { error: err.message });
...
filesRouter.post("/apply", requireRole("admin"), (req, res) => {
  const { id } = req.body || {};
  ...
  recordAudit(req, "file.apply", file.applyPath, result.ok ? "success" : "failure", { id, strategy: file.applyStrategy });
...
filesRouter.post("/restore", requireRole("admin"), (req, res) => {
  ...
  if (mode === "side_by_side") {
    ...
    recordAudit(req, "file.restore", backupFileName, "success", { id, mode: "side_by_side", targetPath: effectiveTargetPath });
    return res.json({
      ok: true,
      id,
      mode: "side_by_side",
  ...
  try {
    ...
    recordAudit(req, "file.restore", backupFileName, "success", { id, mode: "in_place", path: file.path });
    return res.json({
      ok: true,
      id,
      mode: "in_place",
    ...
    recordAudit(req, "file.restore", backupFileName, "failure", { id, error: err.message });
...
filesRouter.post("/restore-and-apply", requireRole("admin"), (req, res) => {
  ...
  recordAudit(req, "file.restore-and-apply", backupFileName, applyResult.ok ? "success" : "failure", { id, path: file.path, strategy: file.applyStrategy });
  const stats = fs.statSync(file.path);
...
filesRouter.post("/backups/mark-known-good", requireRole("operator"), (req, res) => {
  ...
  recordAudit(req, "file.mark-known-good", fileName, "success", { id });
  res.json({ ok: true, knownGood });
});
...
filesRouter.post("/backups/label", requireRole("operator"), (req, res) => {
  ...
  recordAudit(req, "file.label", fileName, "success", { id, label });
...
filesRouter.post("/backups/prune", requireRole("admin"), (req, res) => {
  const { id, mode, keep, days } = req.body || {};
  if (!id || typeof id !== "string") {
    return res.status(400).json({ ok: false, error: "File id is required" });
  }
  ...
  recordAudit(req, "file.prune", id, "success", { mode, deleted: deleted.length });
...
// POST /api/files/register — add a new file entry to the registry (admin+)
filesRouter.post("/register", requireRole("admin"), async (req, res) => {
...
// DELETE /api/files/register/:id — remove a custom file entry (admin+)
filesRouter.delete("/register/:id", requireRole("admin"), async (req, res) => {
  try {
    const { unregisterFile } = await import("../filesRegistry.js");
    unregisterFile(req.params.id);
    res.json({ ok: true });
  } catch (err) {
    res.status(err.message === "Not found" ? 404 : 400).json({ ok: false, error: err.message });
  }
});
